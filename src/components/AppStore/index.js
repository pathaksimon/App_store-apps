...
class AppStore extends Component {
  state = {
    searchInput: '',
    Maintain: appsList[0].category,
  }

  getting = tabId => {
    this.setState({Maintain: tabId})
  }

  changingstatus = event => {
    this.setState({searchInput: event.target.value})
  }

  second2 = () => {
    const {searchInput} = this.state
    const againFilter = appsList.filter(lich =>
      lich.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return againFilter
  }
  ...
  third2 = second1 => {
    ...
    const filteredApps = second1.filter(
      eachSearchedApp => eachSearchedApp.category === Maintain,
    )
  ...
  render() {
    const {Maintain} = this.state
    const second = this.second2()
    const third = this.third2(second)
    return (
      <div className="bg-container">
        <div className="card1">
          <div className="centerist">
            ...
            <div>
              <ul className="uolist2">
                {third.map(aich => (
                  <AppItem apps={aich} key={aich.appId} />
                ))}
              </ul>
            </div>
          </div>
